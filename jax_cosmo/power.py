...
def _halofit_parameters(cosmo, a, transfer_fn):
  ...
  k_nl = 1./R_nl(np.atleast_1d(a)).squeeze()
...
def halofit(cosmo, k, a, transfer_fn,  prescription='takahashi2012'):
  ...
  if prescription=='smith2003':
    ...
    a_n = 10**(1.5222 + 2.8553*n + 2.3706 *n**2 + 0.9903*n**3 +
               0.2250*n**4 - 0.6038*C + 0.1749 * om_de * (1 + w))
    b_n = 10**(-0.5642 + 0.5864*n + 0.5716*n**2 - 1.5474*C +
               0.2279 * om_de * (1 + w))
...
def nonlinear_matter_power(cosmo, k, a=1.,
                           transfer_fn=tklib.Eisenstein_Hu,
                           nonlinear_fn=halofit):
