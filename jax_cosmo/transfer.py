...
def Eisenstein_Hu(cosmo, k, type='eisenhu_osc'):
  ...
  """
  w_m = cosmo.Omega_m * cosmo.h**2
  w_b = cosmo.Omega_b * cosmo.h**2
  ...
  gamma_eff = cosmo.Omega_m*cosmo.h * \
      (alpha_gamma + (1.-alpha_gamma)/(1.+(0.43*k*cosmo.sh_d)**4))
  ...
  if(type == 'eisenhu'):
    ...
    # EH98 (17, 18)
    f = 1.0 / (1.0 + (k * cosmo.sh_d / 5.4)**4)
    Tc = f * T_tilde(k, 1.0, beta_c) + \
        (1.0 - f) * T_tilde(k, alpha_c, beta_c)

    # Baryon transfer function
    # EH98 (19, 14, 21)
    y = (1.0 + cosmo._z_eq) / (1.0 + cosmo._z_d)
    x = np.sqrt(1.0 + y)
    G_EH98 = y * (-6.0 * x +
                  (2.0 + 3.0*y) * np.log((x + 1.0) / (x - 1.0)))
    alpha_b = 2.07 * cosmo._k_eq * cosmo.sh_d * \
        np.power(1.0 + cosmo._R_d, -0.75) * G_EH98

    beta_node = 8.41 * np.power(w_m, 0.435)
    tilde_s = cosmo.sh_d / np.power(1.0 + (beta_node /
                                     (k * cosmo.sh_d))**3, 1.0/3.0)

    beta_b = 0.5 + fb + (3.0 - 2.0 * fb) * np.sqrt((17.2 * w_m)**2 + 1.0)

    # [tilde_s] = Mpc/h
    Tb = (T_tilde(k, 1.0, 1.0) / (1.0 + (k * cosmo.sh_d / 5.2)**2) +
          alpha_b / (1.0 + (beta_b/(k * cosmo.sh_d))**3) *
          np.exp(-np.power(k / cosmo._k_silk, 1.4))) * np.sinc(k*tilde_s/np.pi)
